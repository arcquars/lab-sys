...
*/
	if (diffInHrsWithUnit || diffInMinsWithUnit) {
...
/**
 * join the waiting status objects with open status one, so open shows data for waitings and open
 * 
 * @param {Array} statusTabsList, the array with all the status objects
 * 
 * @return {Array} the array with the status objects with the waiting states joined with the open one  
 */
export const joinWaitingsWithOpenStatus = (statusTabsList) => {
	let openIndex = 0;
	let tabsForShow = [];
	let openApiUrl = "";
	let openCounter = 0;
	for (let index in statusTabsList) {
		// save the Open status tab index in order to set its api url with the one joined with the waiting statuses
		if ('Tancat' === statusTabsList[index].name) {
			tabsForShow.push(statusTabsList[index]);
			openIndex = tabsForShow.length - 1;
			openApiUrl = statusTabsList[index].api_end_point;
			openCounter = statusTabsList[index].tickets_count;
		}
		else if ('Esperant a ser atés' === statusTabsList[index].name || 
				 'Esperant al client' === statusTabsList[index].name) {
			// add to Open status api url the query string for the waiting states, 
			// so when open table is rendered gets Open and waiting statuses together
			openApiUrl += '&'+statusTabsList[index].api_end_point.split('?')[1];
			// update open tickets counter
			openCounter = statusTabsList[index].tickets_count;
			tabsForShow.push(statusTabsList[index]);
			openIndex = tabsForShow.length - 1;
		} else {
	...
	tabsForShow[openIndex].api_end_point = openApiUrl;
	tabsForShow[openIndex].tickets_count = openCounter;
	return tabsForShow;
